...
The runtime now prefers `CompiledModelProvider` for injecting precompiled model payloads from external resources.  
This keeps raw authoring assets outside the live BetterModel server runtime.
...
public class YourPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        BetterModel.compiledModelProvider(() -> List.of(
            new CompiledModelAsset(
                "knight.json",
                () -> Objects.requireNonNull(getResource("knight.json"))
            )
        ));
    }
}
